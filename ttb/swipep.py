...
def swipep(x, fs, plim=numpy.array([30, 5000]), dt=0.001, dlog2p=float(1./48.), dERBs=0.1, woverlap=0.5, sTHR=-numpy.inf):
    fs = float(fs)
    ...
    for i in range(0, len(ws)):
        ...
        """ Compute spectrum """
        w = scipy.signal.windows.hann( int(ws[i]) )         # Hann window
