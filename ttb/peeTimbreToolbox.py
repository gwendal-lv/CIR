...
from . import swipep as swp                # used for single-F0 estimation
from . import my_tools as mt
...
def F_computeAllDescriptor(audio_v, sr_hz, verbose=False):
    """
...
def F_representationFft(audio_v, sr_hz):
    ...
    f_Win_v = scipy.signal.windows.hamming(i_WinSize)
    ...
    X = scipy.fft.fft(f_DistrPts_m, i_FFTSize, axis=0)
...
def ERBpower(a,Fs, hopsize, bwfactor=1.):
    ...
    # power spectrum
    pwrspect = pow(abs(scipy.fft.fft(fr, int(wsize), axis=0)), 2.)
...
def F_computeModulation(f_Env_v, f_ADSR_v, Fs):
    ...
    if flag_is_sustained == 1:
        ...
        fenetre_v    = scipy.signal.windows.hamming(L_n);
        norma        = np.sum(fenetre_v) ;
        fft_v        = scipy.fft.fft(signal_v * fenetre_v*2/norma, N);
        ampl_v         = np.abs(fft_v);
